...
void at_command_transmit_receive (at_command_data* at_command)
{
	...
	{
		HAL_UART_ReceiverTimeout_Config(&GSM_MODEM_HUART, (at_command->timeout * 1000) / 10); // timeout мс * 1000 мс / 10 мкс . 10 мкc = 1 TimeoutValue. 1/115200 = 8.68 ~ 10.0
...
{
	HAL_UART_DMAStop(&GSM_MODEM_HUART);
...
void USART_UART_IDLECallback(UART_HandleTypeDef *huart)
{
    ...
    //HAL_UART_DMAStop(&GSM_MODEM_HUART); //Не отстанавливать работу DMA, т.к. если пришла не ожидаемая посылка, необходимо дальше принимать данные
								 // Отключение DMA будет в прерывании RTO
    ...
    uint8_t uart_rx_counter  = RXBUFSIZE - __HAL_DMA_GET_COUNTER(&GSM_MODEM_DMA_RX);

	//Test function: Print out the received data
    printf("Receive Data(length = %d): %s", (int) uart_rx_counter, "\r\n");
    printf("Data: %s \r\n", modem_rx_buf);
